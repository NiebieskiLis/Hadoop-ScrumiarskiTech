...
create external table Wykonanie_zadania (ID_Wykonanie_zadania int, ID_Sprintu int, ID_Status int, ID_Zadania int, Czas_wykonania double, Estymacja_zadania double, Liczba_committow int, Liczba_zmienionych_plikow int, Liczba_zmienionych_linijek_kodu int) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' STORED AS TEXTFILE;
